...
cli.provideToken()
	.then( token => {
		...
		for ( const file of additionalFiles ) {
			tools.shExec( `cp -R ${ path.resolve( packageRoot, file ) } ${ path.resolve( templatePath, file ) }` );
		}

		log.info( 'Publishing on npm...' );

		// Publish the package on npm.
		tools.shExec( `cd ${ templatePath } && npm publish && cd ${ packageRoot }` );

		// Remove files that were copy.
		for ( const file of additionalFiles ) {
			tools.shExec( `rm -rf ${ path.resolve( templatePath, file ) }` );
		}
